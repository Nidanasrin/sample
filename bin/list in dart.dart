void main() {
  ...
  print(list1);
  print(list1.join(
      " ")); // to remove bracket and add space or anything else btw numbers
  list1.forEach((data) //to make each values in a row
  {
    print(data);
  });
  if (list1.contains(
      12)) //to check the given element or value contains in the list
      {
  ...
  var list4 = List.empty(
      growable: true); // to add values in an empty list we should give growable true
  list4.add(25);
  list4.add(2);
  list4.addAll(list3);
  print(list4);
  ...
  var list6 = List.generate(
      6, (index) => index + 1); //to create a new list with n length
  print(list6);
  var list7 = List.unmodifiable(
      list5); //we can add values of one list to another
